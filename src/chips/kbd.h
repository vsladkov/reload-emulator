...
static inline void kbd_set_active_columns(kbd_t* kbd, uint16_t column_mask) { kbd->active_columns = column_mask; }
// Scan active lines (used together with kbd_set_active_columns
static inline uint16_t kbd_scan_lines(kbd_t* kbd) { return kbd_test_lines(kbd, kbd->active_columns); }
// Set active lines mask (use together with kbd_scan_columns
static inline void kbd_set_active_lines(kbd_t* kbd, uint16_t line_mask) { kbd->active_lines = line_mask; }
// Scan active columns (used together with kbd_set_active_lines
static inline uint16_t kbd_scan_columns(kbd_t* kbd) { return kbd_test_columns(kbd, kbd->active_lines); }
...
void kbd_init(kbd_t* kbd, int sticky_frames) {
    CHIPS_ASSERT(kbd);
    *kbd = (kbd_t){.sticky_time = sticky_frames * 16667};
}
...
void kbd_register_modifier(kbd_t* kbd, int layer, int column, int line) {
    ...
    kbd->mod_masks[layer] =
        (1 << (layer + KBD_MAX_COLUMNS + KBD_MAX_LINES)) | (1 << (column + KBD_MAX_LINES)) | (1 << line);
}
...
void kbd_register_key(kbd_t* kbd, int key, int column, int line, int mod_mask) {
    ...
    CHIPS_ASSERT((line >= 0) && (line < KBD_MAX_LINES));
    kbd->key_masks[key] =
        (mod_mask << (KBD_MAX_COLUMNS + KBD_MAX_LINES)) | (1 << (column + KBD_MAX_LINES)) | (1 << line);
}

// Extract column bits from a 32-bit key mask
static uint16_t _kbd_columns(uint32_t key_mask) { return (key_mask >> KBD_MAX_LINES) & ((1 << KBD_MAX_COLUMNS) - 1); }

// Extract line bits from a 32-bit key mask
static uint16_t _kbd_lines(uint32_t key_mask) { return key_mask & ((1 << KBD_MAX_LINES) - 1); }
...
void kbd_update(kbd_t* kbd, uint32_t frame_time_us) {
    ...
    for (int i = 0; i < KBD_MAX_PRESSED_KEYS; i++) {
        ...
        if (k->released) {
            ...
            if ((kbd->cur_time < k->pressed_time) || (kbd->cur_time > (k->pressed_time + kbd->sticky_time))) {
                k->mask = 0;
                k->key = 0;
                k->pressed_time = 0;
                k->released = false;
